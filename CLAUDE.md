...
2. **Client-side key management**: Users authenticate via seed-phrase-derived AccountId (sr25519). Keys never leave the browser. The login session (address + seed) **is persisted in IndexedDB in plaintext** (`apps/frontend/src/lib/account/sessionStore.ts`) so a reload restores the connection; Disconnect must always clear it. This is a deliberate owner decision (2026-09-12) trading XSS / device-access exposure for usability — do not add encryption-at-rest or session-only behaviour back without asking. Cross-device access requires a user-exported, password-encrypted backup file.
