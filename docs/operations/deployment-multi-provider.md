...
| ストレージのホストごと消滅 (2026-09-12, AWS 撤去) | ✅ ハートビートが 180 秒途絶えると `online` から外れ、fan-out の対象にならない (`NODE_STALE_AFTER_SECS`)。**修正前は永遠に online のままで、死んだ onion 10 台への Tor 接続待ちでタイムラインが Loading... で固まった** |
| ハートビートの跨ホスト伝播 | ✅ core chain-3 → chain-1 → gateway と gossip を中継 (同じハートビートは 2 回目で止まる)。gateway 側で core のストレージが 4 分以上 online を維持することを確認 |
