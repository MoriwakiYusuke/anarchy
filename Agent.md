...
- [x] Moralトークン消費（投稿時に動的コスト: 10 + 0.1×bytes MORAL）
- [x] マルチノードテストネット（3ノード構成）
- [x] 統合テストフレームワーク（ブロック同期、コンセンサス、リカバリ、スケーラビリティ）
- [ ] Identity Pallet（WebAuthn公開鍵管理）
- [ ] WebAuthn署名検証
- [ ] libp2p + Tor統合
...
- [ ] SSS によるデータ断片化
- [ ] ステルスアドレス（DM機能 + 匿名報酬受取）
- [ ] **分散ストレージ報酬（Proof of Spacetime）**
  - データ保持継続ノードへの$moral分配
  - 報酬停止による「経済的忘却」メカニズム
...
- [ ] **反応マイニング（PoW統合版）**
  ...
  - WebWorker + Page Visibility API（フォアグラウンド強制）
  - ステルスアドレス報酬先（名寄せ防止）
- [ ] 動的難易度調整（γ係数の自動調整）
- [ ] ZKP匿名人間証明
